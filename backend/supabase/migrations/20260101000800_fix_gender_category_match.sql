-- Bug fix: `profiles.gender` stores singular values ('man', 'woman',
-- 'nonbinary', 'self_described') but `match_with`/`shown_to` store plural
-- category labels ('men', 'women', 'nonbinary', 'anyone'). The original
-- find_match_candidates compared them directly (c.gender = any(caller.match_with)),
-- so 'man' never equaled 'men' and every non-'anyone' man/woman preference
-- silently matched nobody. Only invisible until now because every prior
-- test used match_with/shown_to = ['anyone'], which short-circuits the
-- comparison entirely.
